...
public class DatabaseHelper extends SQLiteOpenHelper {
  ...
  public String getNameJoins() {
    ...
    String result = "";
    String query = "SELECT "+ COL_NAME + " FROM "+EMPLOYEE_TABLE_NAME + " JOIN " +
            DEPARTMENT_TABLE_NAME + " ON "+EMPLOYEE_TABLE_NAME + "."+COL_ID +
...
//    String[] args = {EMPLOYEE_TABLE_NAME + "."+COL_ID,DEPARTMENT_TABLE_NAME+"."+COL_EMP_ID};
//    Cursor cursor = db.rawQuery(query,args);
  ...
  public String getFullInformation() {
    ...
    SQLiteDatabase db = getReadableDatabase();
    String result = "";
    String query = "SELECT "+ COL_NAME + "," + COL_AGE + "," + COL_ADDRESS +
            "," + COL_SALARY + "," + COL_DEPARTMENT + " FROM "+EMPLOYEE_TABLE_NAME + " JOIN " +
            DEPARTMENT_TABLE_NAME + " ON "+EMPLOYEE_TABLE_NAME + "."+COL_ID +
            " = "+ DEPARTMENT_TABLE_NAME+"."+COL_EMP_ID;
    Cursor cursor = db.rawQuery(query,null);
    ...
    if(cursor.moveToFirst()){
      while(!cursor.isAfterLast()){
        result += cursor.getString(cursor.getColumnIndex(COL_NAME)) + ", who is "+
                ...
                cursor.getString(cursor.getColumnIndex(COL_SALARY)) + ",000 a year working in the "+
                cursor.getString(cursor.getColumnIndex(COL_DEPARTMENT))+" department. ";
        cursor.moveToNext();
      }
    }
    cursor.close();
